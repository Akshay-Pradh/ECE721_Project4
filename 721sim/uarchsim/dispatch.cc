...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // Update instr payload with Active List index
      PAY.buf[index].AL_index = REN->dispatch_inst(dest_valid, log_reg, phys_reg, load_flag, 
                                                   store_flag, branch_flag, amo_flag, csr_flag, pc);
      ...
      if (PAY.buf[index].A_valid){
         // Source A exists, read ready state from instr payload
         A_ready = REN->is_ready(PAY.buf[index].A_phys_reg);
      }
      else {
         // Source A does not exist, ready state = true
         A_ready = true;
      }

      if (PAY.buf[index].B_valid) {
         // Source B exists, read ready state from instr payload
         B_ready = REN->is_ready(PAY.buf[index].B_phys_reg);
      }
      else {
         // Source B does not exist, ready state = true
         B_ready = true;
      }

      if (PAY.buf[index].D_valid) {
         // Source D exists, read ready state from instr payload
         D_ready = REN->is_ready(PAY.buf[index].D_phys_reg);
      }
      else {
         // Source D does not exist, ready state = true
          D_ready = true;
      }
      // FIX_ME #8 END
