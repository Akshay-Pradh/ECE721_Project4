   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      if (PAY.buf[index].checkpoint) {
         ...
         if (PERFECT_BRANCH_PRED) {
            ...
            // FIX_ME #15b BEGIN
            REN->resolve(PAY.buf[index].AL_index, PAY.buf[index].branch_ID, true);     // resolve function of renamer module
            resolve(PAY.buf[index].branch_ID, true);                                   // private function of pipeline_t class
            // FIX_ME #15b END
         }
         else {
      ...
      // VPQ writeback
      if (PAY.buf[index].vp_eligible) SVP->vpq_deposit(PAY.buf[index].vpq_index, PAY.buf[index].C_value.dw);
